...
void EXTI2_IRQHandler(void)
{
	...
	{
        while(KeyLeft); delay_ms(20); while(KeyLeft);
		MoveDir = LEFT;	
...
void EXTI3_IRQHandler(void)
{
	...
	{
        while(KeyRight); delay_ms(20); while(KeyRight);
		MoveDir = RIGHT;
        UserUSB_SendByte(MoveDir);
	}
...
void EXTI9_5_IRQHandler(void)
{
    ...
    if(EXTI_GetITStatus(EXTI_Line9) != RESET)
    {
        ...
        if(KeyUp == 1)	 
        {
            while(KeyUp); delay_ms(20); while(KeyUp);
            DelayN = DelayN + 1;
    ...
    else if(EXTI_GetITStatus(EXTI_Line8) != RESET)
    {
        ...
        if(KeyDown == 1)	 
        {
            while(KeyDown); delay_ms(20); while(KeyUp);
            if(DelayN > 1) DelayN = DelayN - 1;
            else DelayN = 1;
            UserUSB_SendByte(DelayN);
        }
